	...
	public function getNext ($catid, $id, $ordering, $hrefOnly = 0)  {
		...
		foreach ($nextAll as $key => $value) {
			...
			if (isset($value->id) && $value->id > 0) {
				...
				//$href	= JRoute::_('index.php?option=com_phocagallery&view=detail&catid='. $value->catslug.'&id='.$value->slug.$tCom.'&Itemid='. Factory::getApplication()->getInput()->get('Itemid', 1, 'get', 'int'));
