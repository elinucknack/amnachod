...
class CKController {
	...
	static function getInstance($prefix) {
		...
		if (!class_exists($class))
		{
			...
			{
				throw new \InvalidArgumentException(Slideshowck\CKText::sprintf('ERROR_INVALID_CONTROLLER', $type, $format));
			}
		}
		...
		if (!class_exists($class))
		{
			throw new \InvalidArgumentException(Slideshowck\CKText::sprintf('ERROR_INVALID_CONTROLLER_CLASS', $class));
		}
	...
	public function getView($name = '', $type = 'html', $prefix = '')
	{
		...
		if (empty(self::$views[$name][$type][$prefix]))
		{
			...
			{
				throw new \Exception(Slideshowck\CKText::sprintf('ERROR_VIEW_NOT_FOUND', $name, $type, $prefix), 404);
			}
		}
	...
	protected function createView($name, $prefix = '')
	{
		...
		if (!class_exists($viewClass))
		{
			...
			if (!class_exists($viewClass))
			{
				throw new \Exception(Slideshowck\CKText::_('ERROR_VIEW_CLASS_NOT_FOUND : ' . $viewClass . ' - ' . $path), 500);
			}
		}
	...
	public function execute($task) {
		...
		{
			throw new \Exception(Slideshowck\CKText::sprintf('ERROR_TASK_NOT_FOUND', $task), 404);
		}
