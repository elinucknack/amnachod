...
//		$doc->addScript(Slideshowck\CKUri::root(true) . '/media/jui/js/jquery.min.js');
...
//	}
	...
	?>
		<script src="<?php echo Slideshowck\CKUri::root(true) ?>/media/jui/js/jquery.min.js" type="text/javascript"></script>
		<link rel="stylesheet" href="<?php echo Slideshowck\CKUri::root(true) ?>/components/com_slideshowck/assets/font-awesome.min.css" type="text/css" />
	...
	*/
	...
	static function resizeImage($file, $x, $y = '', $thumbpath = 'th', $thumbsuffix = '_th') {
		...
		$thumbfile = str_replace("%20", " ", $thumbfile);
...
//			$thumbfolder = str_replace(basename($file), $thumbpath . "/", $filetmp);
			$thumbfolder = pathinfo($filetmp, PATHINFO_DIRNAME) . "/" . $thumbpath . "/";
			if (!is_dir($thumbfolder)) { 
				mkdir($thumbfolder);
				Slideshowck\CKFile::copy(JPATH_ROOT . '/modules/mod_slideshowck/index.html', $thumbfolder . 'index.html' );
			}
			...
			if ($size['mime'] == 'image/jpeg') {
				...
				$img_big = imagecreatefromwebp($filetmp); # On ouvre l'image d'origine
				$img_new = imagecreate($x, $y);
				# création de la miniature
				$img_mini = imagecreatetruecolor($x, $y) or $img_mini = imagecreate($x, $y);
				// copie de l'image, avec le redimensionnement.
...
//		$item->imgthumb = $item->thumb;
	...
	static function setImageUrl($url) {
		if (strpos($url, 'http') !== 0) {
			$url = Slideshowck\CKUri::root(true) . '/' . trim($url, '/');
		}
	...
	static function getArticle($item) {
		$app = Slideshowck\CKFof::getApplication();
...
//		return $css;
	...
	public static function getProMessage() {
		$html = '<div class="ckinfo"><i class="fas fa-info"></i><a href="https://www.joomlack.fr/en/joomla-extensions/slideshow-ck" target="_blank">' . Slideshowck\CKText::_('SLIDESHOWCK_ONLY_PRO') . '</a></div>';
	...
	public static function getProPreview($imgName) {
		$html = '<div class="ckpropreview">'
					...
					. '<a href="https://www.joomlack.fr/en/joomla-extensions/slideshow-ck" target="_blank">' . Slideshowck\CKText::_('SLIDESHOWCK_ONLY_PRO') . '</a>'
				. '</div>';
