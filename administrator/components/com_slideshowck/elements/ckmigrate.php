...
require_once JPATH_ROOT . '/administrator/components/com_slideshowck/helpers/ckloader.php';
...
//var_dump($options);die;
	...
	protected function isPluginEnabled($plugin = 'slideshowckparams') {
		if (file_exists(JPATH_ROOT . '/plugins/system/' . $plugin)) {
			$isEnabled = CKFof::dbLoadResult("SELECT enabled FROM #__extensions WHERE element = " . CKFof::dbQuote($plugin));
	...
	protected function doMigration($id, $params) {
		...
		$paramsObj = new Slideshowck\CKRegistry($newparams);
